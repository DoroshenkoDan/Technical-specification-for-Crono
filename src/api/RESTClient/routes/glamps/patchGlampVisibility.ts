 ...
 * @example
 * try {
 *   const { data } = await client.api.glamps.patchVisibility({ glamp_id: 1 }, { is_hidden: true });
 *   console.log(data);
 * } catch (error) {
 *   console.error(error);
 * }
...
export interface TAPIPatchGlampVisibility {
...
export type TError = TAPIPatchGlampVisibility['TError'];
export type TSuccess = TAPIPatchGlampVisibility['TSuccess'];
export type TParams = TAPIPatchGlampVisibility['TParams'];
export type TPayload = TAPIPatchGlampVisibility['TPayload'];

export async function patchGlampVisibility(
  this: BaseRESTClient,
  { glamp_id }: TParams,
  payload: TPayload,
