 ...
 *
 * @function patchGlampVerification
 ...
 * @example
 * try {
 *   const { data } = await client.api.glamps.patchVerified({ glamp_id: 1 }, { is_verified: true });
 *   console.log(data);
 * } catch (error) {
 *   console.error(error);
 * }
...
export interface TAPIPatchGlampVerification {
...
export type TError = TAPIPatchGlampVerification['TError'];
export type TSuccess = TAPIPatchGlampVerification['TSuccess'];
export type TParams = TAPIPatchGlampVerification['TParams'];
export type TPayload = TAPIPatchGlampVerification['TPayload'];

export async function patchGlampVerification(
  this: BaseRESTClient,
  { glamp_id }: TParams,
  payload: TPayload,
