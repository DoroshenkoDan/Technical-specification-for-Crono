 ...
 *
 * @function patchGlampPremiumLevel
 ...
 * @example
 * try {
 *   const { data } = await client.api.glamps.patchPremiumLevel({ glamp_id: 1 }, { premium_level: 0 });
 *   console.log(data);
 * } catch (error) {
 *   console.error(error);
 * }
...
export interface TAPIPatchGlampPremiumLevel {
...
export type TError = TAPIPatchGlampPremiumLevel['TError'];
export type TSuccess = TAPIPatchGlampPremiumLevel['TSuccess'];
export type TParams = TAPIPatchGlampPremiumLevel['TParams'];
export type TPayload = TAPIPatchGlampPremiumLevel['TPayload'];

export async function patchGlampPremiumLevel(
  this: BaseRESTClient,
  { glamp_id }: TParams,
  payload: TPayload,
